...
template <typename T, typename Compare = std::less<T>, typename H = std::hash<T>>
class PriorityQueueSpecialized {
public:
	PriorityQueueSpecialized(Compare compare = Compare()) : m_compare(std::move(compare)) {} // ?? Compare()
	...
	std::size_t find(T& t) const {
		return m_set.count(T);
	}
